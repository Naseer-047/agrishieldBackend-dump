...
def log_mrl(pesticide, application_date, harvest_date, estimated_residue, safe_harvest, timestamp):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    """, (pesticide, application_date, harvest_date, estimated_residue, safe_harvest, timestamp))
    conn.commit()
    conn.close()